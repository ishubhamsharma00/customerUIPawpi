  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://pawpi-back-end.onrender.com/auth/signup",
        { credentials: formData }
      );
      console.log(response);

      if (response.data.message === "user already exist and Not verified") {
      ...
      <div className=" hidden lg:block lg:w-1/2  bg-gradient-to-br from-[#EFF3FC] to-[#EFF3FC] rounded-br-[20%] relative   ">
        <div className="h-full">
          ...
          <img
            src={Cartoon}
            alt="Overlayimage"
            className="absolute lg:top-52 lg:left-0 z-10   2xl:top-[500px]  "
          />
        </div>
      </div>
      ...
      <div className="h-auto sm:w-2/3 lg:w-1/2 mx-auto  ">
        ...
        <div className="w-full h-screen  items-center justify-center p-20 lg:p-10 xl:py-44  ">
          ...
          <div className="justify-center text-center w-full">
            <h1 className=" font-Raleway font-bold text-5xl text-[#3C4D64]  ">
              Register Yourself!
            </h1>
            <p className="font-Nunito text-[16px] font-semibold  text-[#3C4D64] py-3">
          ...
          <form className="w-full md:w-4/5 lg:w-4/5 xl:w-3/5 mx-auto border bg-[#3C4D64]  border-[#3C4D64] rounded-xl  hover:shadow-none mb-4 shadow-xl focus:outline-none focus:shadow-outline-blue focus:border-blue-60">
            ...
            {errors.fullname && (
              <p style={{ color: "yellow", marginLeft: "46px" }}>
                {errors.fullname}
              </p>
            )}
          </form>
          ...
          <button
            ...
            className="w-full md:w-4/5 lg:w-4/5 xl:w-3/5  bg-[#3C4D64] mx-auto text-white p-4 mt-8 font-Raleway font-bold text-2xl flex items-center justify-center rounded-2xl hover:bg-slate-700"
          >
            Register
          </button>
        </div>
