...
const NavSm = ({ login, cart, location }) => {
  ...
  return (
    <>
      ...
      <div className="flex fixed bg-white z-50 shadow-xl items-center justify-between h-16 w-full px-1">
        ...
        <div className="h-2/3 w-[40%] flex items-center justify-between gap-3">
            ...
            {authUser ? (
              <>
                <div className="relative">
                  ...
                  >
                    <img
                      src="https://media.licdn.com/dms/image/D4D03AQEDTLNoP8Jxpg/profile-displayphoto-shrink_800_800/0/1684957189400?e=1691625600&v=beta&t=1IMB5rcwp-hSlhFx6qFdt_nzp8IKWdaDpyiPfGZADz8"
                      alt="auth"
                      ...
                      <>
                        <div className="absolute shadow-lg top-10 right-0 w-24 z-10 bg-white flex flex-col gap-2 border-2 rounded-lg font-paw">
                          <div className="flex flex-col gap-2 pl-2 py-4">
                            ...
                            <Link
                              to={"/user/orders"}
                              className="text-sm hover:text-white rounded-md"
                            >
                              My Orders
                            </Link>
          ...
          <ul
            className={`md:hidden shadow-2xl bottom-0 top-16 pb-2 rounded-l-xl 
          ${
            isOpenMenu
              ? "fixed top-0 right-0 w-[70%] h-[90%] duration-300 bg-purple-50 z-10 "
              : "fixed top-0 right-[-100%] w-[70%] h-[90%] bg-purple-50 z-10 duration-300"
          }`}
          >
            <NavLinks />
          </ul>
    ...
    <>
      <div className="flex fixed bg-white shadow-lg z-50 items-center justify-around h-24 w-full px-2">
        <div className="flex relative items-center w-1/6 justify-center">
          <Link to="/">
            <img src={LOGO} alt="LOGO" className="h-24 w-28 scale-150" />
          </Link>
        </div>
        ...
        <div className="flex items-center w-[50%] h-1/2 px-2 gap-1 justify-between bg-navback rounded-md border border-bordergray">
          ...
          <div className="h-8 w-8">
            <img src={SEARCH} alt="searchIcon" className="w-full h-full" />
          </div>
        </div>
        ...
        <div className="h-2/3 w-[27%] flex items-center justify-between">
          ...
          <div className="w-1/3 h-2/3 flex items-center justify-center">
            {authUser ? (
                      ...
                      <>
                        <div className="absolute shadow-lg top-8 right-0 w-32 z-10 bg-white flex flex-col gap-2 border-2 rounded-lg font-paw">
                          <div className="flex flex-col gap-2 items-center py-4">
                            <Link
                              to={"/user/profile"}
                              className="text-md hover:bg-purple-500 px-4 hover:text-white rounded-md py-1"
                            >
                              My Profile
                            </Link>
                            <button className="hover:bg-purple-500 px-4 hover:text-white rounded-md py-1">
                              Sign Out
                            </button>
                          </div>
                        </div>
              ...
              <>
                <div
                  className="flex items-center justify-center w-full h-full bg-purple-500 font-paw text-white text-3xl rounded-md"
                  onClick={login}
                >
                  <h1>{/* <CgProfile /> */}</h1>
                </div>
          ...
          <div
            className="flex cursor-pointer items-center justify-center text-3xl w-[20%]"
            onClick={() => setIsOpenMenu(!isOpenMenu)}
          >
            {isOpenMenu ? (
              <RxCross2 className="text-5xl font-bold" />
            ) : (
          ...
          <ul
            className={`lg:hidden w-[50%] rounded-l-xl bottom-0 top-24 z-10 py-5 bg-purple-50 shadow-2xl
            ${
              isOpenMenu
                ? "fixed top-0 right-0 h-[90%] duration-300"
                : "fixed top-0 right-[-100%] h-[90%] duration-700"
            }`}
          >
            <NavLinks />
          </ul>
    ...
    try {
      if (localStorage.getItem("_id")) {
        const response = await axios.get(
          `https://pawpi-back-end.onrender.com/user/${localStorage.getItem(
            "_id"
          )}`
        );
    ...
    <>
      <div className="fixed h-16 text-xs w-screen bg-white flex items-center shadow-xl py-2 px-5 justify-between">
        <Link to="/" className="h-16 w-32 p-1">
          <img src={yellowLogo} alt="logo" className="h-full w-full" />
        </Link>
        ...
        <div className="flex h-16 w-[30%] text-sm font-semibold tracking-wide  items-center justify-between">
          {links.map((item) => (
            <div
              className={`${
                item.name === selectedItem ? "text-yellow-300" : "text-black"
                      ...
                      <div className="h-full w-4/5 flex-wrap flex justify-start">
                        {item.sublinks.map((subItems) => (
                          <div className="flex my-4 flex-col w-1/5">
                            <Link
                              to={subItems.sublinksid}
                              className="hover:font-extrabold hover:duration-200 duration-200 tracking-wider my-1"
                            >
                              {subItems.Head}
                            </Link>
                            <div className="font-normal text-xs">
                              {subItems.sublink.map((subSubItems) => (
                                <Link
                                  to={`${subItems.sublinksid}/${subSubItems.sublinkid}`}
                                  className="flex flex-col hover:underline hover:underline-offset-1"
                                >
                                  {subSubItems.name}
                                </Link>
                              ))}
                      ...
                      <div className="w-[100%] px-[5%] h-60 flex justify-between gap-16 items-center overflow-x-auto">
                        {Breeds.map((BreedImage) => (
                          <img
                            ...
                            alt={BreedImage}
                            key={BreedImage}
                            className="h-[60%] w-[15%] shadow-xl rounded-xl cursor-pointer bg-gray-50 hover:duration-500 hover:scale-90 duration-500"
                          />
                        ))}
                      </div>
                    )}
                      ...
                      <div className="w-[95%] h-60 flex justify-center">
                        {Lifestage.map((items) => (
                          <img
                            ...
                            alt={items}
                            key={items}
                            className="w-[30%] scale-75 cursor-pointer hover:duration-700 duration-700 hover:scale-90"
                          />
                        ))}
                      </div>
                    )}
            ...
            {authUser ? (
              <>
                <div className="relative w-full">
                  ...
                  >
                    <img
                      src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWk8uZf12aY_Sc1TWTOi5ha_4R6E4zUq28AWKGff7iAX5_pFSizOvu1nBeff56OqrovZY&usqp=CAU"
                      alt="auth"
                    ...
                    {isDropDownOpen && (
                      <>
                        <div className="absolute shadow-lg top-10 right-0 w-40 z-20 bg-white flex flex-col gap-3 p-2 border-2 rounded-lg font-paw">
                          <Link
                            to={"/user/profile"}
                            className="text-lg hover:text-purple-500  hover:border-white border-blue-100 rounded-xl pl-1"
                          >
                            My Profile
                          </Link>
                          <Link
                            to={"/user/profile"}
                            className="text-lg hover:bg-purple-500 hover:text-white  hover:border-white border-blue-100 rounded-xl px-2"
                          >
                            My Profile
                          </Link>
                          <Link
                            to={"/user/profile"}
                            className="text-lg hover:bg-purple-500 hover:text-white border-l-8 hover:border-white border-blue-100 rounded-r-xl pl-1"
                          >
                            My Profile
                          </Link>
                          <Link
                          ...
                          <p
                            className="text-lg cursor-pointer hover:bg-purple-500 hover:text-white border-l-8 hover:border-white border-blue-100 rounded-r-xl pl-1"
                            onClick={() => {
                              setAuthUser(false);
                              localStorage.clear();
                            }}
...
const Navbar = () => {
  ...
  useEffect(() => {
    if (!localStorage.getItem("pincode")) openLocationmodal();
  }, []);
  ...
  return (
    <>
      ...
      <nav>
        ...
        <div
          className={`hidden transition-all ease-in-out lg:flex z-10 overflow-hidden ${
            show
